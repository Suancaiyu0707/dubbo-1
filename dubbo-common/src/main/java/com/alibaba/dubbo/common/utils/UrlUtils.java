...
public class UrlUtils {
    ...
    public static URL parseURL(String address, Map<String, String> defaults) {
        if (address == null || address.length() == 0) {//zookeeper://127.0.0.1:2181
            return null;
        }
        String url;
        if (address.indexOf("://") >= 0) {
            ...
            String[] addresses = Constants.COMMA_SPLIT_PATTERN.split(address);//将address根据逗号','进行拆分
            url = addresses[0];
            if (addresses.length > 1) {//如果配置了多个地址，则除了第一个地址，其它的作为备用地址
        ...
        String defaultProtocol = defaults == null ? null : defaults.get("protocol");//设置默认的协议protocol
        if (defaultProtocol == null || defaultProtocol.length() == 0) {
            defaultProtocol = "dubbo";
        }//获取默认配置username/password/port
    ...
    //解析注册地址并创建对应的url对象，并根据默认配置设置相应的属性值
    //zookeeper://127.0.0.1:2181->zookeeper://127.0.0.1:2181/com.alibaba.dubbo.registry.RegistryService?application=demo-provider&dubbo=2.0.0&pid=28878&qos.port=22222&timestamp=1527387892739
