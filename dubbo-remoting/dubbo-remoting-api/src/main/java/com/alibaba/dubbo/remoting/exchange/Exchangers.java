...
public class Exchangers {
    ...
    public static ExchangeServer bind(URL url, ExchangeHandler handler) throws RemotingException {
        ...
        url = url.addParameterIfAbsent(Constants.CODEC_KEY, "exchange");//替换编码器为exchange
        return getExchanger(url).bind(url, handler);//获得网络传输的转换类，默认是HeaderExchanger，然后绑定到对应的url
    }
    ...
    public static Exchanger getExchanger(URL url) {
        String type = url.getParameter(Constants.EXCHANGER_KEY, Constants.DEFAULT_EXCHANGER);//获取网络传输的转换类，默认是HeaderExchanger,可以通过exchanger进行配置,但是必须要有响应的拓展类
        return getExchanger(type);
    }

    /***
     * "header" -> "class com.alibaba.dubbo.remoting.exchange.support.header.HeaderExchanger"
     * 获得交换者拓展类
     * @param type(这里是header)
     * @return
     */
